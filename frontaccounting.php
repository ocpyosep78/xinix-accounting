 ...
 * mobile applications 
 */
include_once($path_to_root . '/applications/mobile_reporting.php');
...
class front_accounting {
    ...
    function display() {
        global $path_to_root;
        if ($this->detect->isMobile()) {
            include($path_to_root . "/themes/".user_theme()."/mobile_renderer.php");
            $this->init();
            $rend = new renderer();
            $rend->wa_header();
    ...
    function init() {
         ...
         */
        $this->add_application(new mobile_reporting_app());
